...
def init_db():
    ...
    if cursor.fetchone()[0] == 0:
        sample_tables = [
            ('Table 1', 2, 'Main Hall', 'Cozy two-seater near the window.'),
            ('Table 2', 4, 'Main Hall', 'Family table with a view of the courtyard.'),
            ('Table 3', 4, 'Terrace', 'Open-air terrace seating under the vines.'),
            ('Table 4', 6, 'VIP Room', 'Private room, perfect for larger groups.'),
            ('Table 5', 2, 'Terrace', 'Romantic terrace seating for two.'),
            ('Table 6', 8, 'Main Hall', 'Large table for celebrations and parties.'),
        ]
        cursor.executemany(
            "INSERT INTO restaurant_tables (table_number, capacity, location, description) VALUES (?, ?, ?, ?)",
            sample_tables
        )
...
def api_tables_availability():
    ...
    for t in tables:
        result.append({
            ...
            'has_photo': bool(t['image_data']),
            'image_url': url_for('media_table', id=t['id']) if t['image_data'] else '',
